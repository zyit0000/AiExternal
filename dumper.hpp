...
#ifdef __arm64__
    // On Apple Silicon, pointers have PAC bits in the upper range
    return (pointer >> 47) ? (pointer & 0x00007FFFFFFFFFFFULL) : pointer;
#else
    // On Intel, pointers are clean; do not mask them
    return pointer;
#endif
}

inline bool is_valid_pointer(uint64_t pointer) {
    // Basic sanity check for macOS user-space pointers
    return pointer >= 0x10000 && pointer <= 0x00007FFFFFFFFFFFULL;
}
